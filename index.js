...
async function scrapeGoogleMaps(query, limit = 10) {
    ...
    try {
        ...
        // --- SCROLLING PHASE ---
        // If limit is 0, we scroll until endOfList is true.
        while ((limit === 0 || results.length < limit) && !endOfList) {
        ...
        const processCount = limit > 0 ? Math.min(places.length, limit) : places.length;



        let lastAddress = null; // Track previous address to detect stale data
        ...
        for (let i = 0; i < processCount; i++) {
            ...
            // Scroll into view
            // Scroll into view (gentle alignment)
            try {
                await item.evaluate(el => el.scrollIntoView({ block: 'nearest', inline: 'center' }));
                await delay(500);
            } catch (e) { }

            // NAVIGATION STRATEGY (Robust):
            // 1. Attempt Standard Click
            // 2. Wait for URL change.
            // 3. If timeout, attempt JS Click (fallback)
            // 4. Wait again.

            try {
                await item.click();
            } catch (e) { /* ignore click error, move to retry */ }
            ...
            try {
                ...
                // Timeout? Try JS Click
                console.log(`  > Navigation slow/failed. Retrying with JS Click for item ${i}...`);
                try {
                    await item.evaluate(el => el.click());
                    await page.waitForFunction(
                        (prev) => window.location.href !== prev && window.location.href.includes('/place/'),
                        { timeout: 8000 }, // Give it more time on retry
                        previousUrl
                    );
                    urlChanged = true;
                } catch (e2) {
                    console.log(`Warning: Navigation timeout for item ${i} after retry. URL might not have changed.`);
                }
            }
            ...
            // 3. ADDRESS CHANGE WAIT (Anti-Duplication):
            // Even if H1 updates, the details (address) might lag. 
            // We wait for the address to be DIFFERENT from the previous iteration.
            if (lastAddress) {
                try {
                    await page.waitForFunction(
                        (lastAddr) => {
                            const addrBtn = document.querySelector('button[data-item-id="address"]');
                            // If no address button yet, keep waiting (or it might be a business without address)
                            // If it exists, check if it matches previous.
                            if (!addrBtn) return true; // If missing, maybe it's a new layout, let's proceed (risky but prevents stuck)
                            // Actually, better: if present, ensure it's DIFFERENT.
                            const currentAddr = addrBtn.getAttribute('aria-label') || "";
                            return currentAddr !== lastAddr;
                        },
                        { timeout: 3000 },
                        lastAddress
                    );
                } catch (e) {
                    // Timeout means address didn't change (or is same). 
                    // We log waring but proceed, as some businesses share addresses.
                    console.log(`Warning: Address didn't change from previous item (Possible stale data or shared location).`);
                }
            }
            ...
            const data = await page.evaluate(async (nameFromLink) => {
                ...
                // Category
                let category = '';
                const categoryBtn = queryMain('button[jsaction*="category"]');
                if (categoryBtn) category = categoryBtn.innerText.trim();

                // DEBUG CATEGORY
                if (!category) {
                    const allBtns = Array.from(mainContainer.querySelectorAll('button'));
                    const btnDebug = allBtns.map(b => `${b.innerText} (${b.getAttribute('jsaction')})`).join('\n');
                    category = "DEBUG_SEE_FILE";
                    return { ...null, debug_text: `Category Missing. Buttons:\n${btnDebug}\n\nFull Text:\n${mainContainer.innerText}` };
                }
                ...
                return { name, rating, reviews, address, phone, website, category, businessClaimed };
            }, nameFromLink);
            ...
            // Update lastAddress for next iteration comparison
